# Lab: 高层次综合调度实现
在这个Lab里，实现一个高层次综合的调度算法，处理没有控制流情况下的调度问题。你需要根据输入的数据流图（DFG）、资源约束和额外限制，确定每个操作的开始执行周期，使得总延迟最小化，同时满足时钟周期和资源限制。
...
第一行输入一个自然数q,表示一共有q条额外限制。

接下来q行,每行三个整数,$op_0, op_1, difference$。

对应限制$\text{start\_cycle}(op_0)-\text{start\_cycle}(op_1)\leq difference$,注意difference可以为负数。这里的$op_i$表示dfg文件中的第$op_i+1$行对应的操作。
...
```
需要满足$\text{start\_cycle}(op3)-\text{start\_cycle}(op1)\leq 1$

## 输出格式

一共k行,每行一个正整数,表示第几周期运行。
...
`vector<Constr*> $constsr` 保存了所有constraint的信息，
...
- 运行 `make test TEST=[1..10]` 运行对应测试点，生成的schedule.txt文件会保存你的调度结果。
...
- 当 `Constraint` 约束没有被满足时，会报告违反约束的两个运算，以及他们被调度的周期

当所有检查通过或只违反 `Constraint` 约束时，verifier会报告最终的latency和对应的分数。你将会看到：
```
================
||    PASS    ||
================
...
最终测试实例包括下发的open test和若干hidden test，每个测试点的运行时间不超过 `5 min`。
每个测试点的评分规则如下：

- 当你的调度结果错误或者运行超时，你在该问题上的得分为0。
- 当你的调度结果正确但是违反了constraint限制，则你在该问题的得分为50。
- 当你的调度结果正确并且满足constraint限制时，你在该问题上的得分为 $\text{max}(60, 100*(\text{latency}/{satsdc}))$。 即固定SAT+SDC算法结果为100分，其他结果按照线性函数计算，不低于60分。

**最终保证有一半数据Constraint数量为0。**

## Minisat求解器使用

实现SDC+SAT需要调用SAT求解器，下发代码中提供了minisat作为可能用到的SAT求解器。
...
最终提交的文件为`scheduler.cpp`。

请把`scheduler.cpp`这一个文件放在服务器的`~/hw/lab2`目录下，确保运行`ls ~/hw/lab2`时输出如下所示，不要提交多余的文件。
