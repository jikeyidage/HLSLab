...
void dfs_cp(DFG* dfg, const vec2d<int>& uses, int x, int in_cycle, double delay, double target, vector<int>& critical) {
...
void verify_constrs_failed(DFG* dfg, int pred, int succ, int difference) {
    printf("================\n");
    printf("||   FAILED   ||\n");
    printf("================\n");

    printf("Stmt %d start at cycle %d\n", pred, dfg->stmts[pred - 1]->start_cycle);
    printf("Stmt %d start at cycle %d\n", succ, dfg->stmts[succ - 1]->start_cycle);
    printf("The difference should be no larger than %d\n", difference);
}

bool verify_constraint(DFG* dfg, const vector<Constr*>& constrs) {
    printf("Verifying Constraints....\n");
    int n_stmts = dfg->stmts.size();

    for (auto constr : constrs) {
        if (dfg->stmts[constr->op_0 - 1]->start_cycle - dfg->stmts[constr->op_1 - 1]->start_cycle >
            constr->difference) {
            verify_constrs_failed(dfg, constr->op_0, constr->op_1, constr->difference);
            return false;
        }
    }
    return true;
...
int main(int argc, char** argv) {
    if (argc < 4) {
        printf("Usage: %s path/to/dfg path/to/op path/to/constr [path/to/schedule]\n", argv[0]);
        return -1;
    }
    ...
    int sat_sdc_lat;
    if (fscanf(dfg_file, "%d", &sat_sdc_lat) != 1) {
        sat_sdc_lat = -1;
    }
    ...
    if (!verify_constraint(&dfg, constrs)) {
        printf("Score: 50\n");
        return 0;
    }
    ...
    int total_latency = 0;
    for (int i = 0; i < n_stmts; ++i) {
        total_latency = max(dfg.stmts[i]->start_cycle + max(dfg.stmts[i]->op->latency, 1) - 1, total_latency);
    }
    ...
    if (sat_sdc_lat > 0) {
        printf("Score: %.2lf\n", max(60.0, sat_sdc_lat * 100.0 / total_latency));
    }
